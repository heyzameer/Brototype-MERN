...
export default function HostLogin() {
   ...
   * Updates inline error states and returns true if validation passes.
   */
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/auth/host/login`, { // NOTE: Changed to /auth/host/login
        method: 'POST',
      ...
      if (response.ok && data.data && data.data.token) {
        ...
        // NOTE: Updated redirection to /dashboard/host as this is a Host login page
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl p-12">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-2">Login to your buisness account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            ...
            <div className="text-right mt-2">
              <a href="/forgot-password/host" className="text-indigo-600 hover:text-indigo-800 text-sm font-medium">Forgot password?</a>
            </div>
          </div>
          <button
            type="submit"
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-600">
            ...
            <button
              onClick={() => window.location.href = '/register/host'}
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          ...
          <div className="space-y-1 text-xs text-gray-600">
            {/* You may want to add host-specific test account info here */}
          </div>
        </div>
      </div>
    </div>
