...
export default class HostSigninUseCase {
  constructor(
  ...
  private async sendMail(email: string, name: string) {
    ...
    console.log("host signin use case");
    console.log(otp);
  ...
  async refreshAccessToken(refreshToken: string): Promise<{ accessToken: string }> {
    ...
    const accessToken = this.tokenService.createAccessToken({ email, id, role: UserRole.Host });
