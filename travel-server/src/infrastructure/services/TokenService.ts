...
export default class TokenService implements ITokenService {
  ...
  generateToken(id: string, role: UserRole): { accessToken: string; refreshToken: string } {
    const accessToken = this.createAccessToken({ email: id, id, role });
    const refreshToken = this.createRefreshToken({ email: id, id });
    return { accessToken, refreshToken };
  }

  clearRefreshToken(email: string): Promise<void> {
    // In a real application, this would involve invalidating the refresh token in a database or cache.
